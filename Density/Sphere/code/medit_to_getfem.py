...
class Medit2D:
    ...
    def load(self, filename):
        ...
        dim_index = content.index("Dimension")+1
        if int(content[dim_index]) != self.dimension:
            print(f"Wrong dimension : got {int(content[dim_index])} instead of {self.dimension}")
    ...
    def save_solution(self, sol, filename, sol_type=None):
        ...
        f.write("MeshVersionFormatted 1\n\n")
        f.write("Dimension {}\n\n".format(self.dimension))
        f.write("SolAtVertices\n{}\n{} {}\n\n".format(nbdof, 1,sol_type)) # 1 pour le nombre de champ, sol_type pour le type de champ (1 pour scalaire, 3 pour tenseur)
    ...
    def solution_to_surface(self, sol):
        #
        # Get the solution in sol (array of the right dimension)
        # and interpret it as a third dimension of the mesh.
        # Returns a MeditS
        #

        # We copy self into a MeditS
        surface = MeditS()

        surface.dimension = 3

        surface.nb_vertices = np.copy(self.nb_vertices)
        surface.vertices = np.copy(self.vertices)
        surface.vertices_labels = np.copy(self.vertices_labels)

        surface.nb_triangles = np.copy(self.nb_triangles)
        surface.triangles = np.copy(self.triangles)
        surface.triangles_labels = np.copy(self.triangles_labels)

        surface.nb_edges = np.copy(self.nb_edges)
        surface.edges = np.copy(self.edges)
        surface.edges_labels = np.copy(self.edges_labels)

        # We add a dimension to the vertices
        surface.vertices.resize((3, surface.nb_vertices))
        surface.vertices[2,:] = sol
    ...
    def extract_region(self, region):
        ...
        # Renumber the vertices in the submesh.triangles array
        to_values = -np.ones(self.nb_vertices, dtype=np.int64) # Vertices indices can't take negative values so it's safe
        to_values[vertices_still_present] = np.arange(1,submesh.nb_vertices+1)
        submesh.triangles = to_values[submesh.triangles-1]

        # Relabel the edges (we keep potentially labels of edges that doesn't exists anymore)
        if self.edges_labels is not None :
            submesh.edges = to_values[self.edges-1]
            submesh.edges_labels = np.copy(self.edges_labels) if self.edges_labels is not None else None
            submesh.nb_edges = self.nb_edges
...
class Medit3D:
    ...
    def load(self, filename):
        ...
        if int(content[dim_index]) != 3:
            print("Wrong mesh dimension; try using the MeditS class if the mesh is a 2D/Surface mesh.")
            return
        ...
        # We build the array of edges
        #
        if "Tetrahedra" in content :
            tetras_index = content.index("Tetrahedra")+1
            self.nb_tetras = int(content[tetras_index])

            current_index = tetras_index + 1

            self.tetras = np.zeros((4, self.nb_tetras),dtype=int) # List of tetras
            self.tetras_labels = np.zeros(self.nb_tetras, dtype=int)     # List of the region ID of the triangles

            for i in range(self.nb_tetras):
                str_list = content[current_index:current_index+4]
                self.tetras[:,i] = [int(elt) for elt in str_list]
                self.tetras_labels[i] = int(content[current_index+4])
                current_index += 4 + 1 # The +1 accounts for the reference
    ...
    def save(self, filename):
        ...
        # Write the edges
        #
        if self.tetras is not None :
            f.write("\nTetrahedra\n{}\n".format(self.nb_tetras))

            str_to_format = "{} {} {} {} {}\n"
            for i in range(self.nb_tetras):
                f.write(str_to_format.format(*(self.tetras[:,i]), self.tetras_labels[i]))
    ...
    def from_getfem(self, gf_mesh):
        ...
        self.nb_tetras = gf_mesh.max_cvid()+1
        self.tetras = np.zeros((4, self.nb_tetras), dtype=np.int64)
        self.tetras_labels = np.zeros(self.nb_tetras, dtype=np.int64)

        start = time.time()
        for i in range(self.nb_tetras):
            self.tetras[:,cvid[i]] = gf_mesh.pid_from_cvid(cvid[i])[0]+1 # +1 for the medit indexation
        ...
        for rid in gf_mesh.regions():
            ...
            for j in range(nb_convex_in_region):
                ...
                # Add the whole triangle if fid = -1
                if fid == 65535 : # = -1 mod 2^16. Getfem must encode it on an unsigned int
                    self.tetras_labels[cvid] = rid

                # Else add only the edge
                else:
                    triangles.append(gf_mesh.pid_in_faces(cvfids[:,j])+1)
                    triangles_labels.append(rid)


        self.triangles = np.array(triangles).T
        self.triangles_labels = np.array(triangles_labels)
        self.nb_triangles = self.triangles_labels.size
    ...
    def to_getfem(self):
        ...
        # Create the mesh (getfem indexation begins at 0)
        gf_mesh = gf.Mesh('ptND', self.vertices, self.tetras-1)

        # We set the different regions for the tetrahedra
        cvid = gf_mesh.cvid()

        # List all the labels
        if self.tetras_labels is not None :
            unique_labels = np.unique(self.tetras_labels)
            if self.triangles_labels is not None :
                unique_labels = np.concatenate((unique_labels, np.unique(self.triangles_labels)))

        # WARNING : EN FAISANT COMME CA, L'INDICE 0, S'IL APPARAIT DANS LES
        # EDGES ET DANS LES TRIANGLES, SE VERRA DONNER DEUX INDICES DIFFERENTS
        if self.tetras_labels is not None :
            for label in np.unique(self.tetras_labels):
                labeled_cv = cvid[self.tetras_labels == label]
                labeled_cv = np.array([ cvid[self.tetras_labels == label], -1*np.ones(labeled_cv.size)])
                if label == 0:
                    gf_mesh.set_region(np.amax(unique_labels)+1, labeled_cv)
                    print(f"Warning : assigning label {np.amax(unique_labels)+1} to GetFEM region instead of 0")
                else:
        ...
        if self.triangles_labels is not None :
            for label in np.unique(self.triangles_labels):
                ...
                # We replace it with the maximum of the labels + 1
                if label == 0:
                    labeled_faces = gf_mesh.faces_from_pid(self.triangles[:,self.triangles_labels == label]-1)
                    gf_mesh.set_region(np.amax(unique_labels)+1, labeled_faces)
                    print(f"Warning : assigning label {np.amax(unique_labels)+1} to GetFEM region instead of 0")
                else:
                    labeled_faces = gf_mesh.faces_from_pid(self.triangles[:,self.triangles_labels == label]-1)
...
def animate_2d_eigenvalue(mesh_file, sol_file, out_folder, duration, fps):
    #
    # Ok on finit ça après
    #
    mesh2d = Medit2D().load(mesh_file)
    sol2d = 0.1*mesh2d.load_solution(sol_file)

    nframes = int(duration*fps)

    for i in tqdm(range(nframes)):
        meshS = mesh2d.solution_to_surface(np.cos(2*np.pi*i/nframes)*sol2d)
        meshS.save(f"{out_folder}/surf.{i}.mesh")
        meshS.save_solution(np.cos(2*np.pi*i/nframes)*sol2d, f"{out_folder}/surf.{i}.sol")

#
# Old functions (but may still be useful, who knows ?)
#
